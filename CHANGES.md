...
- Root cause: same failure class as the AWS MCP Server tool-naming bug fixed in Round 35, but in the built-in subagent-invocation tool. Confirmed empirically that its literal callable name is NOT stable — this session's own tool list exposes it as `orchestrate_subagent`, while this repo's steering/prompts hardcode `use_subagent`, and the user's other session hit `invoke_sub_agent`. Three different names for the same capability across sessions/versions

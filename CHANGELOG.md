...
- **`README.md`** — new `## Installation` section directly after the intro (before Prerequisites/Local Tooling): the 🚀 AI-assisted install (open by default) is the primary path, 📦 Manual Installation follows, and the macOS one-shot script stays under Local Tooling as a fallback. Previously the install blocks sat below the tooling table.
...
- **`steering/kiro-cli-v3-migration.md`** actually _recommended_ the invalid `tools: [*]` — corrected to `tools: ["*"]` with a bare-star warning; fixed the backup location (`agents/v2-backup/`), documented the correct invocation (`kiro-cli chat --v3` / `--agent-engine v3`), and noted that `agent list` / `agent validate` are V2/JSON-only (they can't see `.md` agents — expected, not the bug).

...
For full-app builds, delegate freely to the configured subagents (frontend, serverless, testing, architect, devops, data, security, docs, image-gen) via the subagent-invocation tool (name varies by session — see `steering/AGENTS.md` if a specific name fails). You're the orchestrator; let specialists do specialist work in parallel — up to 4 at once is typical for this config, but not a hard limit; scale up if the task genuinely benefits.

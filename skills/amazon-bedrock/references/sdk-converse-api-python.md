...
- **Prompt management**: Pass prompt ARN as `modelId` — it _replaces_ the model ID, not alongside it. When using managed prompts, MUST NOT include `inferenceConfig`, `system`, `toolConfig`, or `additionalModelRequestFields` (baked into the prompt). Messages are _appended_ after the prompt's messages, not replacing them.
